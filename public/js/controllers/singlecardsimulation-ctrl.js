angular.module('app').controller('ExpenseandreceiptsListController',
    ['$rootScope', '$scope','$uibModal','$location','$schedulingService','$profile',function($rootScope, $scope,$modal,$schedulingService,$location,$profile) {
        $scope.$on('$viewContentLoaded', function() {
            ...
            $scope.initData = function (arg){
                ...
                $profile.getRequest('/trials/singlecardsimulation/init').then(function(data){
                    ...
                    // jQuery("#jqGrid-simu").jqGrid("clearGridData");    //清空表数据
                    // jQuery("#jqGrid-simu").trigger("reloadGrid");      //重新加截
                    jQuery("#jqGrid-simu").GridUnload();                  //刷新重载
                    jQuery("#jqGrid-d-simu").GridUnload();
                    //写gqgrid的表头，星期
                        ...
                        ondblClickRow : function(rowid,iRow,iCol,e) {
                            ...
                            if (rowid ) {
                                ...
                                var row = $scope.dataSource[rowid-1];
                                        $profile.open('/trials/singlecardsimulation/scheduling/row/edit','EditSchedulingCtrl','',{row:row,columns:$scope.colums},true).result.then(function(row){
                                            $scope.initData(true);
                                        })
                            }
                        },
                    ...
                    for(var i = 0;i<= $scope.dataSource.length;i++){
                        jQuery("#jqGrid-simu").jqGrid('addRowData', i + 1, $scope.dataSource[i]);
                    }
                    //排期汇总表填充数据
                    $scope.getSumData();

                    //组出单头上的星期
                    if(arg) jQuery("#jqGrid-simu").jqGrid('setGroupHeaders', {
                    ...
                    if(arg) jQuery("#jqGrid-d-simu").jqGrid('setGroupHeaders', {
                        useColSpanStyle: true,
                        groupHeaders:[
                            {startColumnName: 'T-1',align: 'center', numberOfColumns: 1,  titleText: $scope.jqgridweek['T-1']},
                            {startColumnName: 'T0',align: 'center', numberOfColumns: 1, titleText: $scope.jqgridweek['T0']},
                    ...
                    //设定分页
                    jQuery("#jqGrid-simu").jqGrid('navGrid', '#gridpager-simu', {edit : false,add : false,del : false,search:false,refresh:false});
                    //设定窗口冻结
                    jQuery("#jqGrid-simu").jqGrid('setFrozenColumns');
                    jQuery("#jqGrid-d-simu").jqGrid('setFrozenColumns');
                    // 设定CheckBox勾选事件
            ...
            $scope.genScheduling = function() {
                $profile.confirm('确定','确定将已勾选卡片的初排转成已排',function(){
                    ...
                    var selectRows = jQuery("#jqGrid-simu").jqGrid('getGridParam', 'selarrrow');
                    if(selectRows != null){
            ...
            $scope.getSumData = function(){
                jQuery("#jqGrid-d-simu").jqGrid('addRowData', 1, $scope.total[0]);
                jQuery("#jqGrid-d-simu").jqGrid('addRowData', 2, $scope.total[1]);
                jQuery("#jqGrid-d-simu").jqGrid('addRowData', 3, $scope.total[2]);
            ...
            $scope.gettotal3 = function(){
                ...
                $("#jqGrid-d-simu").jqGrid('clearGridData');//清空表格
                $("#jqGrid-d-simu").jqGrid('setGridParam',{
                    datatype:'local',
...
app.controller('EditSchedulingCtrl', ['$scope','$uibModalInstance','$profile','action',
            ...
            $profile.confirm('确定','确定重排已选中的计划吗？',function() {
                if (isValid)
                    $profile.post('/trials/singlecardsimulation/scheduling/regeneration',{
                        head:$scope.row,columns:$scope.columns,reGenerationStatus:"0"
                    }).then(function (data) {
                        if (data.success) {
                            $profile.alter(data.message, function () {
                                // $profile.getRequest('/trials/singlecardsimulation/init').then(function(data){
...
app.service('$schedulingService',['$http','$profile',function($http,$profile){

    this.regeneration = function(row,columns){
        var l_yes1 = "N";
        var l_yes2 = "N";
        for(var i = 0; i < columns.length; i++) {
            //日期重排如果勾选上，那当天的金额一定要勾上纳入重排
            if (columns[i].datechk == "Y") {
                columns[i].pricechk = "Y";
                l_yes2 = "Y";
            }
            if (columns[i].pricechk == "Y") {
                l_yes1 = "Y";
            }
        }
        if(l_yes1 == "N" || l_yes2 =="N"){
        ...
        $profile.confirm('确定','确定重排已选中的计划吗？',function() {
            if (isValid)
                $profile.post('/trials/singlecardsimulation/scheduling/regeneration',{
                    head:row,columns:columns,reGenerationStatus:"0"
                }).then(function (data) {
                    if (data.success) {
                        $profile.alter(data.message, function () {
                            // $profile.getRequest('/trials/singlecardsimulation/init').then(function(data){
